...
The sender gate is separate from OpenAI. Configure `EMAIL_ALLOWED_SENDER_DOMAINS` as exact comma-separated domain names without wildcards, schemes, or addresses. Optionally configure `EMAIL_ALLOWED_SENDER_ADDRESSES` with exact comma-separated envelope addresses, which is useful when a custom mailbox alias forwards with a Gmail envelope sender. Both are Supabase Edge Function secrets; neither is a Cloudflare Worker variable nor part of an OpenAI request.
