...
Raw email requests use a dedicated `EMAIL_INGEST_TOKEN`, not a plant ingest token. Before reading the body or calling OpenAI, the Edge Function requires the envelope sender's exact lowercase domain to appear in the comma-separated `EMAIL_ALLOWED_SENDER_DOMAINS` secret. Subdomains are not implicit. The function then validates relay metadata, parses MIME generically, and sends every valid PDF candidate to the OpenAI Responses API in one in-memory structured-analysis request. Email text and filenames are context only. The analyzer currently recognizes `green-tariff-receipt`; unknown or ambiguous messages fail without a storage write, causing the Worker to forward the original email to its fallback address.
...
The sender gate is separate from OpenAI. Configure `EMAIL_ALLOWED_SENDER_DOMAINS` only as a Supabase Edge Function secret, using exact comma-separated domain names without wildcards, schemes, or addresses. This value is neither a Cloudflare Worker variable nor part of an OpenAI request.
