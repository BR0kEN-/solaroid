...
class Pvgis {
  static readonly BaseQuery = Object.entries(
    {
      usehorizon: '1',
      userhorizon: '',
  ...
  static buildUrl(panel: Solaroid.Supabase.Plant.Pv.Field) {
  ...
  static async getProjection(metadata: Solaroid.Supabase.Plant.Metadata): Promise<Solaroid.Supabase.Pvgis.Projection | null> {
    ...
    await Promise.all(
      panels.map(async (panel) => {
        const response = await fetch(this.buildUrl(panel))
